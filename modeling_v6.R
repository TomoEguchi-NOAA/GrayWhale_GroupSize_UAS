...
model.ver <- "v6"
model.file <- "models/model_Pois_Gam_logitP_v6.txt"
# switch(model.ver,
...
jags.params <- c("B0", "B1", "B2", "B3", "B4", "p.Vis", "mu.Vis",
...
out.file.name <- paste0("RData/jm_out_", model.ver, "_alpha_max6.rds")
...
params.to.plot <- c("B0", "B1", "B2", "B3", "sigma.Obs", "alpha.", "beta.")
...
#                          "v6" = c("B0", "B1", "B2", "B3", "sigma.Obs", "alpha.", "beta."),
...
if (model.ver == "v6" | model.ver == "v7" | model.ver == "v8"){
  ...
  ggsave(p.pred,
         filename = paste0("figures/GroupSizePredictions_", model.ver, "_alpha_max6.png"),
         device = "png", dpi = 600)
  ...
  ggsave(p.pred.Vis,
         filename = paste0("figures/Vis_GroupSizePredictions_", model.ver, "_alpha_max6.png"),
         device = "png", dpi = 600)
}
...
ggsave(p.pred.UAS,
       filename = paste0("figures/UAS_GroupSizePredictions_", model.ver, "_alpha_max6.png"),
       device = "png", dpi = 600)
...
out.v6 <- plot.ridges.2(out.file.name,
                        bandwidth = 0.3)
# 
# 
