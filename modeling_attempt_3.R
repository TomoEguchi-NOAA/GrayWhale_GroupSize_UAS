...
# 
# 
# In this version, I model each group size recorded by visual observer separately. In other words,
# models are fit to n_{vis} = 1, n_{vis} = 2, ..., separately.
...
for (k in 1:length(group.sizes)){
  ...
  out.file.name <- paste0("RData/jm_out_", model.ver, "_max6_n", group.sizes[k], ".rds")
